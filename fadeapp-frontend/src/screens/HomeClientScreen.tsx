    ...
    const load = async () => {
      try {
        ...
        if (!data || !Array.isArray(data)) {
          console.warn("⚠️ El backend no devolvió un arreglo válido");
        } else if (data.length === 0) {
          console.warn("⚠️ No se encontraron barberías para la ciudad:", city);
        } else {
          console.log(`✅ ${data.length} barberías recibidas`);
        }
